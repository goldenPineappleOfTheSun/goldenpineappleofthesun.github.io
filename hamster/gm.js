...
function step() {
	...
	for (let s in unnamedObjects){
		...
		let first = unnamedObjects[f];
		let second = unnamedObjects[s];
		...
		if (first.x < second.x + second.sprite.width 
			&& first.x + first.sprite.width > second.x 
			&& first.y < second.y + second.sprite.height 
			&& first.y + first.sprite.height > second.y) {
...
function Mesh(obj, material, x, y, z, w, h, src){
...
function Model(obj){
	...
	this.add = function(name, src, options){
		...
		options.side = options.side || THREE.FrontSide;
		...
		let mesh = new Mesh(this.root, material, options.x, options.y, options.z, options.w, options.h, src)
...
function ModelBuilder(name, src, options){
	...
	this.add = function(name, src, w, h, options){
		options = options || {};
		options.w = w || 1;
		options.h = h || 1;
